module.exports = {
		...
		<html>
		 ...
		 <body>
		   ...
		   <div class="search_box">
			   <input type="text" placeholder="교수명 입력">
			   <button>검색</button>
			 </div>
		   </article>
		   <footer>
	...
	department_HTML:function(college){
		return `
		<!DOCTYPE html>
<html>
 <head>
 ...
 <body>
   ...
   <section>
	...
	list3:function(notice_list) {
			...
			while( i <notice_list.length) {
				var num= i+1;
				list = list + `<tr>`;
				list = list+`<td>`+ num+`</td>` +'<td><a href="'+ `http://15.164.103.153:3000/noticeboard?id=`+`${notice_list[i].ntboard_id}`+'">' 
  ...
  list7:function(field_list){
    ...
    while(i<field_list.length){
      list = list + `<input name="id"  value = "` + `${field_list[i].field_id}`+ `" type="checkbox" >`+`${field_list[i].field_name}` ; 
      i = i + 1 ;
    }
