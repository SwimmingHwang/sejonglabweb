...
module.exports = {
	/*로그인 전 페이지*/
	basic_HTML:function(list,field,notice,free,college_create,ntboard_create,frboard_create, login ) {
		return `
   ...
   <section>
	...
	list4:function(free_list){
		...
		while( i <free_list.length) {
			var num= i+1;
			list = list + `<tr>`;
			list = list+`<td>`+ num+`</td>` +'<td><a href="'+ `/freeboard?id=`+`${free_list[i].frboard_id}`+'">' 
		   ...
		   <link rel="stylesheet" type="text/css" href="/stylesheets/style2.css">
			...
			<body>
			  ...
			  <div>
				|개인 정보 및 연구실 정보 수정
				<form action="/prof_management" method="post">
				   Name: <input type="text" value="${list[0].professor_name}" name="name"> </input><br>
				   Email: <input type="text" value="${list[0].professor_email}" name="email"></input><br>
				   URL: <input type="text" value="${list[0].professor_url}" name="url"></input><br>
				   Office Hour: <input type="text" value="${list[0].professor_officeHour}" name="office_hour"></input><br>
				   Lab location: <input type="text" value="${list[0].lab_location}" name="lab_location"></input><br>
				   Lab name: <input type="text" value="${list[0].lab_name}" name="lab_name"></input><br>
				   Lab tel: <input type="text" value="${list[0].lab_tel}" name="lab_tel"></input><br>
				   <input type="submit">
				</form>
				<br>
				|프로젝트 정보 수정
